# Next.js and React Troubleshooting Guide
...
**Problem:** Server components trying to use client-side features
...
**Problem:** API routes not found or returning errors

```typescript
// Common mistakes in app/api/hello/route.ts

// Bad: Wrong export names
export function get() { // ❌ Should be GET
...
export async function POST(request: Request) {
  const data = await request.json() // ❌ Could throw
...
function BadImage() {
  return <img src="/large-image.jpg" alt="Large image" /> // ❌ No optimization
}
...
**Problem:** Multiple state updates not batching properly
...
function BadExample() {
  ...
  const handleClick = async () => {
    // These cause separate re-renders in React 17
    setCount(1)
    setName('John')
  }
}
...
**Problem:** Missing dependencies or infinite re-renders
...
function GoodEffect({ userId }) {
  ...
  useEffect(() => {
    fetchUser(userId).then(setUser) // ✅ userId in dependency array
  }, [userId])
}

// Problem: Object in dependency causing infinite re-renders
function BadObjectDep({ config }) {
  useEffect(() => {
    // ❌ config object changes reference every render
    doSomething(config)
  }, [config])
}

// Solution: Use useMemo or destructure specific properties
function GoodObjectDep({ config }) {
...
### 3. Key Prop Issues in Lists

**Problem:** Incorrect key props causing render issues
...
function GoodList({ items }) {
  return (
    <ul>
      {items.map(item => (
        <li key={item.id}>{item.name}</li> // ✅ Unique ID as key
      ))}
    </ul>
  )
}
...
**Problem:** Not cleaning up subscriptions or timers
...
# Build with type checking disabled (not recommended)
...
const OptimizedChild = memo(function ChildComponent({ value, onUpdate }) {
  return <div onClick={() => onUpdate(value)}>{value}</div>
})
