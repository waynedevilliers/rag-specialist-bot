...
- Return a single parent element or Fragment
...
const ExpensiveComponent = memo(function ExpensiveComponent({ name, count }: Props) {
  // This component only re-renders when name or count changes
  return (
    <div>
      <h2>{name}</h2>
      <p>Count: {count}</p>
